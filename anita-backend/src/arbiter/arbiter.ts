...
export async function resolveAction(
  ...
  【当前玩家状态】
  - 当前所在舱段: ${playerState.currentRoom || '未定位'}
  - 已探索区域: ${(playerState.exploredRooms ?? []).join(', ') || '仅当前区域'}
  ...
  3. 空间移动判定：若玩家动作包含前往新区域/穿越气闸/探索相邻舱段，结合当前所在舱段及剧本物理连通规则判定。若允许前往，在 newRoom 填入目标舱段ID；若被门禁阻挡、通路断绝或未尝试移动，填 null。
  4. 纯客观：你只需输出裁决数据，绝对不要输出任何第一人称台词。
